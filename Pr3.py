...
class Fettuccine:
    def __init__(self, pasta, parmesan, shrimp, сhampignons, onion, punch, basil, salmon, chicken):
        self.pasta = pasta 
        self.parmesan = parmesan 
        self.shrimp = shrimp 
        self.сhampignons = сhampignons 
        self.onion = onion 
        self.punch = punch 
        self.basil = basil 
        self.salmon = salmon 
    ...
    def __str__(self):
        return f"Fettuccine with {self.shrimp}, {self.сhampignons}, {self.onion}, {self.punch}, {self.basil}, {self.salmon}, and {self.chicken}"

def check_login(username, password):
    mydb = mysql.connector.connect(host="localhost", database="Fettucci", user="sa", password="123" )
    cursor = mydb.cursor()
...
def new_login(username,password):
    ...
    values = (username,password, bal,10,10000)
...
def gethistorycountprosto(username):
    mydb = mysql.connector.connect(host="localhost", database="ShFettucciauRai", user="sa", password="123")
    cursor = mydb.cursor()
    query = "SELECT history_count FROM polsovatel WHERE Login_polsovatel = %s;"
    cursor.execute(query, [username])
    result = cursor.fetchone()
...
def check_phone(phone):
    mydb = mysql.connector.connect(host="localhost", database="Fettucci", user="sa", password="123")
    cursor = mydb.cursor()
    query = "select history_summ from polsovatel where phone_number = %s" %phone
...
role = False
#TODO Для входы 
print("Добро пожаловать в '\Фитучи'\n")
print("1. Авторизация\n"
      "2. Регистрация\n")
polz = 0
command = input()
    ...
    while(vixod == False):
        ...
        if(command == 1):
            ...
            if(phone==""):
                ...
                print()
            elif(15000>int(check_phone(phone)) and int(check_phone(phone))>5000):
                print("Ваша скидка 5%")
                summa = summa - summa * 5 / 100 
            elif(25000>int(check_phone(phone)) and int(check_phone(phone))>15000):
                print("Ваша скидка 10%")
                summa = summa - summa * 10 / 100 
            elif(int(check_phone(phone))>25000):
    ...
    while(vixod == False):
        ...
        if(vibor == 1):
            ...
            if(phone==""):
                ...
                print("Бронзовая карта 5% скидка") 
            elif(25000>int(check_phone(phone)) and int(check_phone(phone))>15000):
                print("Серебрянная карта 10% скидка")
            elif(int(check_phone(phone))>25000):
                print("Платиновая карта 20% скидка")
            else:
